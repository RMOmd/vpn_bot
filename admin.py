...
async def manage_user_vpn(callback: types.CallbackQuery, bot: Bot, **kwargs):
    ...
    try:
        ...
        keyboard = [
            ...
            [
                InlineKeyboardButton(
                    text="🚫 Деактивировать пользователя" if is_active_status else "✅ Активировать пользователя",
...
async def toggle_user(callback: types.CallbackQuery, bot: Bot, **kwargs):
    ...
    try:
        ...
        if user:
            ...
            await manage_user_vpn(callback, bot)
        else:
            await callback.answer("Пользователь не найден")
    except Exception as e:
        logger.error(f"Ошибка при переключении пользователя: {e}")
        await callback.answer("Произошла ошибка при переключении пользователя")
    finally:
        session.close()
...
async def process_user_search(message: types.Message, bot: Bot, state: FSMContext, **kwargs):
    ...
    try:
        ...
        # Создаем кнопку для активации/деактивации пользователя
        toggle_button = [InlineKeyboardButton(
            text="🚫 Деактивировать" if user.is_active else "✅ Активировать",
            callback_data=f"user_toggle_{user.telegram_id}"
        )]

        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔍 Искать другого", callback_data="users_search")],
            toggle_button,  # Добавляем кнопку для активации/деактивации
...
async def process_server_country(callback: types.CallbackQuery, state: FSMContext, bot: Bot, **kwargs):
    ...
    # Исправляем извлечение ID страны из callback_data
    parts = callback.data.split('_')
    if len(parts) >= 3:
        try:
            country_id = int(parts[2])  # Формат server_country_ID
        except ValueError:
            await callback.answer("Ошибка в формате ID страны")
            return
    else:
        await callback.answer("Некорректный формат callback_data")
        return
...
def register_admin_handlers(dp, bot: Bot):
    ...
    dp.callback_query.register(wrap_handler(toggle_user), lambda c: c.data.startswith("user_toggle_"))
